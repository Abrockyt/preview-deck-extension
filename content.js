/* ══════════════════════════════════════════════════════════════
   ...
   Re-injection is expected: chrome.scripting.executeScript runs this
...
/* Document-space rect. Pins are positioned in document coordinates, not
   viewport ones, so they stay on their element while the page scrolls.
...
function onClick(e) {
  ...
  chrome.runtime.sendMessage({
    ...
    tag: {
      ...
      styles: stylesFor(el),
      /* The live viewport at the moment of tagging: the panel uses this
         to pick which captured device screenshot to crop from. */
...
function onKey(e) {
  if (e.key === 'Escape' && state.armed) {
    disarm();
    chrome.runtime.sendMessage({ type: 'PD_DISARMED' }).catch(() => {});
  }
}
