/* ══════════════════════════════════════════════════════════════
   Store logo generator — Preview Deck

   The Edge Add-ons "Store listings" page wants a logo separate from
   the package's toolbar icons (which top out at 128px) — 300×300 is
   the size it asks for. Same mark, same generator technique as
   tools/make-icons.js, just rendered larger; duplicated rather than
   imported for the same reason package-firefox.js duplicates the zip
   writer — no module wiring in a project with no build step.

   Run:  node tools/make-store-logo.js
   ══════════════════════════════════════════════════════════════ */
...
function shade(u, v) {
  ...
  if (inPortGap && !inPortOuter) return [r, g, b, 255];
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      ...
      for (let sy = 0; sy < SS; sy++) for (let sx = 0; sx < SS; sx++) {
        ...
        const px = shade(u, v); if (!px) continue;
        const a = px[3] / 255;
        R += px[0] * a; G += px[1] * a; B += px[2] * a; A += a;
      }
      const total = SS * SS, o = (y * size + x) * 4;
      if (A > 0) {
        out[o] = Math.round(R / A); out[o + 1] = Math.round(G / A);
        out[o + 2] = Math.round(B / A); out[o + 3] = Math.round((A / total) * 255);
      }
    }
  }
...
const file = path.join(dir, 'store-logo-300.png');
fs.writeFileSync(file, render(300));
console.log(`  icons/store-logo-300.png  ${fs.statSync(file).size} bytes`);
